...
class Test_Matrix_UI:
    ...
    def test_matrix_change_drill_correlation_layer(self,job_id,epcam_ui_start,
        ...
        job_info = {}
        matrix_file_path = MyODB.get_odb_matrix_file_path(file_compressed_path)  # 获取odb的layer信息
    ...
    def test_matrix_double_click_layer_has_step(self, job_id, epcam_ui_start,
        ...
        self.matrix = PageMatrix()
        self.matrix.double_click_layer_has_step() # 双击有step的layer单元格

        self.view_graphic = PageViewGraphic()
        assert self.view_graphic.is_right()
    ...
    def test_matrix_double_click_step(self, job_id, epcam_ui_start,
        ...
        self.matrix = PageMatrix()
        self.matrix.double_click_step()  # 双击有step的layer单元格

        self.graphic = PageGraphic()
        img_name = 'matrix_to_graphic_window'
        cute_coords = [25, 799, 8, 1374] # 截图坐标，后面的是水平方向
        img_standard_str = "graphic\\" + img_name + "_cut_standard.png"
        self.graphic.is_right(img_name, cute_coords, img_standard_str)
    ...
    def test_matrix_click_layer(self, job_id, epcam_ui_start,
        ...
        self.matrix = PageMatrix()
        self.matrix.click_layer()  # 单击layer

        img_name = 'click_layer'
        cut_coords = [25, 799, 8, 1032]
        img_standard_str = "matrix\\" + img_name + "_cut_standard.png"
        assert self.matrix.is_right(img_name, cut_coords, img_standard_str)
    ...
    @pytest.mark.coding
    @pytest.mark.parametrize("job_id", GetTestData().get_job_id('Change_Matrix'))
    def test_matrix_rectangle_selection_layer(self, job_id, epcam_ui_start,
                                download_file_compressed_entity_filter_delete_all_jobs_import):
        """
        选中多行layer后显示工具栏
        禅道用例ID：4183
        ...
        pyautogui.moveTo(550, 670, duration=2)
        pyautogui.mouseUp(button='left')

        img_name = 'rectangle_selection_layer'
        cut_coords = [25, 799, 8, 1032]
        img_standard_str = "matrix\\" + img_name + "_cut_standard.png"
        self.matrix.is_right(img_name, cut_coords, img_standard_str)
