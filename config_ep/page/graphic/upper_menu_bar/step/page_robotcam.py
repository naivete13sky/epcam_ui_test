...
class PageRobotCAM(object):
    def __init__(self):
        self.graphic_window = RunConfig.driver_epcam_ui.window(**page.graphic_window_para)

        # 切换到RobotCAM子窗口
        self.robotcam_window = self.graphic_window.child_window(
            **upper_menu_bar.robotcam_window_para)

        self.temp_path = RunConfig.temp_path_base

    def capture_image(self, img_name):
        self.robotcam_window.set_focus()  # 激活窗口
        time.sleep(0.1)
        drill_correlation_layer_pic = self.robotcam_window.capture_as_image()  # 截图
    ...
    def close(self):
        self.robotcam_window.child_window(title="关闭", control_type="Button").click_input()

    def save(self):
        self.robotcam_window.click_input(coords=upper_menu_bar.robotcam_save_button_coords)
    ...
    def information_click_ok_button(self):
        ...
        self.information_window = self.graphic_window.child_window(
            **upper_menu_bar.robotcam_information_window_para)
        self.information_window.click_input(coords=upper_menu_bar.robotcam_information_ok_button_coords)