...
class PageMRC(object):
    def __init__(self):
        self.graphic_window = RunConfig.driver_epcam_ui.window(**page.graphic_window_para)

        # 切换到RobotCAM子窗口
        self.mrc_window = self.graphic_window.child_window(
            **upper_menu_bar.mrc_window_para)

        self.temp_path = RunConfig.temp_path_base

    def capture_image(self, img_name):
        self.mrc_window.set_focus()  # 激活窗口
        time.sleep(0.1)
        drill_correlation_layer_pic = self.mrc_window.capture_as_image()  # 截图
    ...
    def close(self):
        self.mrc_window.child_window(title="关闭", control_type="Button").click_input()

    def click_run_globally_button(self,time_sleep=0.5):
        self.mrc_window.click_input(coords=upper_menu_bar.mrc_run_globally_button_coords)
        time.sleep(time_sleep)

    def open_viewer_result_window(self,time_sleep=0.5):
        self.mrc_window.click_input(coords=upper_menu_bar.mrc_viewer_result_button_coords)
        time.sleep(time_sleep)
