...
class PageChangeSymbol(object):
    def __init__(self):
        self.graphic_window = RunConfig.driver_epcam_ui.window(**page.graphic_window_para)

        # 切换到change_symbol窗口
        self.change_symbol_window = self.graphic_window.child_window(
            **upper_menu_bar.edit_change_symbol_window_para)

        self.temp_path = RunConfig.temp_path_base

    def capture_image(self, img_name):
        self.change_symbol_window.set_focus()  # 激活窗口
        time.sleep(0.1)
        drill_correlation_layer_pic = self.change_symbol_window.capture_as_image()  # 截图
    ...
    def close(self):
        """点击change_symbol窗口的关闭"""
        self.change_symbol_window.child_window(title="关闭", control_type="Button").click_input()

    def symbol_name(self, symbol):
        """symbol文本框输入名称"""
        self.change_symbol_window.click_input(coords=upper_menu_bar.edit_change_symbol_parameter_coord)
        send_keys(symbol)

    def click_ok_button(self):
        self.change_symbol_window.click_input(coords=upper_menu_bar.edit_change_symbol_ok_button_coord)