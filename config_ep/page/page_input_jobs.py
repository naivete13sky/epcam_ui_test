...
class PageInputJobs(object):
    def __init__(self):
        ...
        # 切换到input_jobs_window窗口
        self.input_jobs_window = self.engineering_input_window.child_window(
            **page.engineering_file_input_jobs_window_para)

        self.temp_path = RunConfig.temp_path_base

    def capture_image(self, img_name):
        self.input_jobs_window.set_focus()  # 激活窗口
        time.sleep(0.1) # 等待窗口激活
        drill_correlation_layer_pic = self.input_jobs_window.capture_as_image()  # 截图
    ...
    def close(self):
        self.input_jobs_window.child_window(title="关闭", control_type="Button").click_input()